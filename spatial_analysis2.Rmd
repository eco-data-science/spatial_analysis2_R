...
* `readShapePoly()` doesn't automatically read in CRS info from a .prj file; you have to manually coerce the coordinate reference system.
...
![SpatialPolygonsDataFrame structure](imgs/spdf_sketch.png)
...
NOTE: When accessing the `@data` slot, keep in mind that all of the polygon information is in the `@polygons` slot - and the order of rows in `@data` needs to stay matched up with the order of the list in `@polygons`.  Changing the order of rows (or adding/deleting rows) in `@data` DOES NOT make analogous changes to `@polygons`...
...
You should also be able to use `dplyr`- and `tidyr`-type functions like `mutate()` and `group_by()` (since `group_by()` doesn't reorder the rows); just be careful of things like `filter()`, `arrange()`, `summarize()` that might reorder or add or delete rows.

``` {r}
poly_bc_rgn@data <- poly_bc_rgn@data %>%
  mutate(h_kg   = h_tonnes * 1000,
...
plot(poly_bc_rgn, col = rgb(.7, .7, 1), border = rgb(0, 0, 1))
plot(poly_bc_mpa, col = rgb(1, .5, .5, .5), border = NA, add = TRUE)
...
The `rgeos` package has many of the standard geoprocessing tools available, for example: `gArea()`, `gDifference()`, `gIntersection()`, `gBuffer()`...
...
* Create a near-shore MPA layer by intersecting the MPA layer with the 3 nautical mile offshore layer
* Find the total area of the near-shore ocean, from the coastline out to 3 nautical miles, for each of BC's regions.
* Find the area of MPAs within 3 nm, and divide it by the total near-shore area.

``` {r}
### Load in the 3 nm buffer layer
poly_bc_3nm <- readShapePoly(fn = file.path(dir_spatial, 'ohibc_offshore_3nm_simple2'), proj4string = p4s_bc)

### Load in a different MPA layer - drastically simplified and in the proper
### CRS, to make the gIntersection go faster...
poly_bc_mpa2 <- readShapePoly(fn = file.path(dir_spatial, 'mpa_bc_simple8'), proj4string = p4s_bc)

### Intersect the MPA layer with the 3 nm regions layer.  This 
poly_bc_3nm_mpa <- gIntersection(poly_bc_3nm, poly_bc_mpa2)
```

``` {r}
area <- lapply(poly_bc_3nm@polygons, function(x) sapply(x@Polygons, function(y) y@area))
...
for (i in 1:length(area)) {
  cat(sprintf('rgn %s: largest polygon %.5f (%s)\n', poly_bc_3nm@data$rgn_id[i], max(area[[i]]), poly_bc_3nm@data$rgn_nam[i]))
} 
...
for(i in 1:length(mainPolys)){
  if(length(mainPolys[[i]]) >= 1 && mainPolys[[i]][1] >= 1){
    poly_bc_3nm@polygons[[i]]@Polygons  <- poly_bc_3nm@polygons[[i]]@Polygons[mainPolys[[i]]]
    poly_bc_3nm@polygons[[i]]@plotOrder <- 1:length(poly_bc_3nm@polygons[[i]]@Polygons)
  }
}

poly_bc_3nm@plotOrder <- 1:length(poly_bc_3nm@polygons)

slot(poly_bc_3nm, "polygons") <- lapply(slot(poly_bc_3nm, "polygons"), 
   checkPolygonsHoles) 

writeOGR(poly_bc_3nm, dsn = dir_spatial, layer = 'ohibc_offshore_3nm_simple2', driver = 'ESRI Shapefile', overwrite_layer = TRUE)
...
* Smaller raster cells = more accurate results, but slower.  Let's go with 2500 m cells.
...
rast_bc_mpa <- rasterize(poly_bc_mpa2, rast_base, field = 'STATUS_YR', fun = 'min')
```
...
![raster::extract() image](imgs/rasterextract.png)
...
``` {r}
mpa_rgn_summary <- mpa_rgn_df %>%
  group_by(rgn_id) %>%
  summarize(cells_mpa = sum(!is.na(year)),
            cells_tot = n(),
            pct_mpa   = round(cells_mpa/cells_tot, 3) * 100) %>%
  left_join(poly_bc_rgn@data %>%
              select(rgn_id, rgn_name),
            by = 'rgn_id')

knitr::kable(mpa_rgn_summary)
```
...
gIsValid(poly_bc_rgn, byid = TRUE) 
  ### checks valid geometries; in this case, one polygon feature
  ### has invalid geometries... how to fix it? cleangeo!
