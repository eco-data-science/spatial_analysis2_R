...
`tmap` uses `tm_shape()` to define the object to be plotted, similar to `ggplot()`; and then layer assignments, similar to `geom_line()` or `geom_boxplot()` etc.

See here for more: https://cran.r-project.org/web/packages/tmap/vignettes/tmap-nutshell.html

```{r plotting polygons in tmap, echo = TRUE, eval = TRUE}
library(tmap)
library(RColorBrewer)
