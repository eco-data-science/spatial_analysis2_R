...
For this workshop/tutorial, we will work through a simplified analysis to examine the proportion of British Columbia's coastal waters that are protected by marine protected areas (MPAs).  This is based on the Ocean Health Index _Lasting Special Places_ sub-goal of the [_Sense of Place_ goal](http://ohi-science.org/goals/#sense-of-place).

# Setup

For info on setting up for the workshop, [please visit the readme.md in the repository.](https://github.com/eco-data-science/spatial_analysis2_R/blob/gh-pages/readme.md)
...
```{r read_shapefiles_method2, echo = TRUE, eval = TRUE}
...
```{r viewing_SpatialPolygonsDataFrame_slots, echo = TRUE, eval = TRUE}
...
```{r adding_data_to_SpatialPolygonsDataFrame, echo = TRUE, eval = TRUE}
...
```{r selecting_and_filtering_by_attribute, echo = TRUE, eval = TRUE}
...
```{r fixing_projections, echo = TRUE, eval = TRUE}
...
```{r set_up_base_raster, echo = TRUE, eval = TRUE}
...
```{r rasterize_MPA_polygons, echo = TRUE, eval = TRUE, warning = FALSE}
...
```{r raster_extract, echo = TRUE, eval = TRUE, warning = FALSE}
...
```{r name_list_elements_from_raster_extract, echo = TRUE, eval = TRUE}
...
```{r convert_list_elements_to_dataframe, echo = TRUE, eval = TRUE}
...
``` {r gdal_rasterize_example, echo = TRUE, eval = FALSE}
library(gdalUtils)
...
``` {r identifying_and_fixing_errors, echo = TRUE, eval = FALSE}
...
```{r ggplot_for_maps, echo = TRUE, eval = TRUE}
library(ggplot2)
...
The `tmap` package uses a similar grammar to `ggplot2`, but seems way more intuitive for plotting maps.  You build up a map layer by layer, without having to do anything special to manipulate the polygons as you do for ggplot.
...
```{r tmap_basemap, echo = TRUE, eval = TRUE}
library(tmap)
library(RColorBrewer)

head(poly_bc_rgn@data, 3) ### what attributes can we plot?
#                rgn_name rgn_id rgn_code h_tonnes   h_kg h_tot h_prop
...
Add a few extra layers to the base map; each of these layers directly accesses the attribute table (@data) in our BC harvest spatial polygons object.  You can also add additional shapes (e.g. we could add a layer based on the BC MPA spatial polygons object).
...
Map styles and small multiples are easy to do; and there is plenty of direct control on the formatting of the legend and other elements of the map.
``` {r tmap_styling}
...
### small multiples by passing a vector of variables to a layer
...
### format the legend of the base map
...
### instead of ggsave:
save_tmap(bc_map_legend, "img/bc_map.png", width=1200, height=1000)

```

Here's an example of a map created using `tmap`, with all the relevant code from the page above:
![world map](img/world_map.png)

## Other R spatial viz awesomeness
